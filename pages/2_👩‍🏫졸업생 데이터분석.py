...
def draw_kemss(df):
    ...
    plt.title("불/합격에 따른 국영수과사 등급과 대학")
    st.pyplot(plt)

def draw_all(df):
    plt.figure(figsize=(10, 6))
    sns.catplot(data=df, x="불합", y="전과목", hue="대학", kind="bar")
    plt.title("불/합격에 따른 전과목 등급과 대학")
    st.pyplot(plt)
...
def draw_k(df):
    ...
    plt.title("불/합격에 따른 국어 등급과 대학")
    st.pyplot(plt)

def draw_e(df):
    plt.figure(figsize=(10, 6))
    sns.catplot(data=df, x="불합", y="영어", hue="대학", kind="bar")
    plt.title("불/합격에 따른 영어 등급과 대학")
    st.pyplot(plt)
...
def draw_m(df):
    ...
    plt.title("불/합격에 따른 수학 등급과 대학")
    st.pyplot(plt)

def draw_so(df):
    plt.figure(figsize=(10, 6))
    sns.catplot(data=df, x="불합", y="사회", hue="대학", kind="bar")
    plt.title("불/합격에 따른 사회 등급과 대학")
    st.pyplot(plt)
